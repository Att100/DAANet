    ...
    for i in range(0,num_gt):
        ...
        for j in range(0,num_rs_dir):
            ...
            try:
                ...
                try: 
                    rs = io.imread(rs_dir_lists[j]+gt_name.split(".")[0]+".jpg") # read the corresponding mask from each method
...
def compute_pre_rec(gt,mask,mybins=np.arange(0,256)):
    ...
    if(gt.shape!=mask.shape):
        mask = np.array(Image.fromarray(mask).resize((gt.shape[1], gt.shape[0])))
        if(len(mask.shape)>2):
            mask = mask[:,:,0]
...
#       F-measure (beta) 'FM': numpy array with shape of (num_rs_dir, 256)
    ...
    for i in range(0,num_gt):
        ...
        for j in range(0,num_rs_dir):
            ...
            try:
                rs_name = gt_name
                if "NLDF" in rs_dir_lists[j]:
                    rs_name = gt_name.split(".")[0]+"_NLDF.png"
                rs = io.imread(rs_dir_lists[j]+rs_name) # read the corresponding mask from each method
                rs = mask_normalize(rs)*255.0 # convert rs to [0,255]
            except IOError:
                try: 
                    rs = io.imread(rs_dir_lists[j]+rs_name.split(".")[0]+".jpg") # read the corresponding mask from each method
                    rs = mask_normalize(rs)*255.0 # convert rs to [0,255]
                except:
                    print('ERROR: Couldn\'t find the following file:',rs_dir_lists[j]+rs_name)
                    continue
            try:
...
def plot_save_pr_curves(PRE, REC, method_names, lineSylClr, linewidth, linestyle, xrange=(0.0,1.0), yrange=(0.0,1.0), dataset_name = 'TEST', save_dir = './', save_fmt = 'pdf'):

    fig1 = plt.figure(1)
    num = PRE.shape[0]
    for i in range(0,num):
        if (len(np.array(PRE[i]).shape)!=0):
            plt.plot(REC[i], PRE[i],lineSylClr[i],linewidth=linewidth[i],label=method_names[i], linestyle=linestyle[i])
...
def plot_save_fm_curves(FM, mybins, method_names, lineSylClr, linewidth, linestyle, xrange=(0.0,1.0), yrange=(0.0,1.0), dataset_name = 'TEST', save_dir = './', save_fmt = 'pdf'):

    fig2 = plt.figure(2)
    num = FM.shape[0]
    for i in range(0,num):
        if (len(np.array(FM[i]).shape)!=0):
            plt.plot(np.array(mybins[0:-1]).astype(np.float)/255.0, FM[i],lineSylClr[i],linewidth=linewidth[i],label=method_names[i], linestyle=linestyle[i])
